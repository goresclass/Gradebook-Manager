...
export function GradebookProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    saveTimerRef.current = setTimeout(() => {
      ...
      try { localStorage.setItem(STORAGE_KEY, payload); } catch { /* non-critical */ }
    }, 500);
  ...
  const setActiveClass = useCallback((id: string) => { setActiveClassId(id); }, []);
  ...
  const moveStudentToPeriod = useCallback((studentId: number, targetClassId: string): boolean => {
    ...
    setClasses(prev => {
      ...
      moved = true;
      return prev.map(c => {
        if (c.id === activeClassId) return { ...c, rows: c.rows.filter(r => r.id !== studentId) };
        if (c.id === targetClassId) return { ...c, rows: [...c.rows, student] };
        return c;
      });
    });
    return moved;
  ...
  const updateRunRecord = useCallback((studentId: number, runId: string, updates: Partial<Pick<RunRecord, "label" | "mileTime">>) => {
    updateActiveClass(c => ({
      ...c,
      rows: c.rows.map(r =>
        r.id === studentId
  ...
  const importMileTimes = useCallback((text: string): { updated: number; notFound: number; error?: string } => {
    try {
      ...
      if (timeIdx === -1) return { updated: 0, notFound: 0, error: "Could not find a mile time column" };
      ...
      for (const line of lines.slice(1)) {
        ...
        entries.push({ roll: rollIdx >= 0 ? vals[rollIdx]?.trim() : undefined, sid: idIdx >= 0 ? vals[idIdx]?.trim() : undefined, mileTime });
      }
  ...
  const restoreBackup = useCallback((newClasses: ClassRecord[], newActiveId: string) => {
    ...
    const maxId = safe.length > 0 ? Math.max(...safe.flatMap(c => c.rows.map((r: StudentRow) => r.id || 0))) : 0;
    if (maxId > _uid) _uid = maxId;
    setClasses(safe);
  ...
  const avg = graded.length ? Math.round(graded.reduce((s, r) => s + (r.score ?? 0), 0) / graded.length) : null;
  const dist: [number, number][] = [100, 90, 80, 70, 65, 50].map(s => [s, graded.filter(r => r.score === s).length]);

  return (
    <GradebookContext.Provider value={{
      classes, activeClassId, setActiveClass, addClass, deleteClass,
      rows, ready, className, setClassName, updateRow, addRow, deleteRow, clearAll,
      importCSV, importMileTimes, withScores,
      stats: { total: rows.length, graded: graded.length, special: special.length, avg, dist },
      archiveRuns, deleteRunRecord, updateRunRecord, renameRunLabel, addRunRecord,
      moveStudentToPeriod, restoreBackup,
    }}>
      {children}
